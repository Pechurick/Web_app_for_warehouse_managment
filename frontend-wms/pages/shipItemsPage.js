export async function renderShipItemsPage() {
    const root = document.getElementById('root');
    root.innerHTML = `
        <h1>Відправлення товарів за заявками</h1>
    ...
    try {
        const response = await fetch(
            'http://localhost:3000/orders/status/checked'
        );
        if (!response.ok)
        ...
        document.querySelectorAll('.start-pick-btn').forEach((btn) => {
            btn.addEventListener('click', () => {
                ...
                renderShippingItemProcessPage(orderId);
            });
        });
    } catch (error) {
...
export async function renderShippingItemProcessPage(id) {
    const root = document.getElementById('root');
    root.innerHTML = `
        <h1>Завантаження товарів за заявкою ${id}</h1>
    ...
    try {
        ...
        if (data.length === 0) {
            slideContent.innerHTML = '<p>Немає позицій для завантаженя.</p>';
            return;
        }
                ...
                .addEventListener('click', async () => {
                    ...
                    try {
                        const res = await fetch(
                            `http://localhost:3000/ship-item/order/${id}`,
                            {
                                method: 'POST',
